...
export default function GrupoDetailPage() {
    ...
    const fetchGrupoData = async () => {
        try {
            ...
            const miembrosWithProfiles = miembrosData.map(m => ({
                ...m,
                profile: profiles?.find(p => p.id === m.user_id)
            }))

            setMiembros(miembrosWithProfiles)
        } catch (error) {
                           ...
                           ${index === 0 ? 'bg-[#ffd700]/5' : ''}`}
                            >
                                ...
                                <div className="text-right">
                                    <div className={`text-xl font-black ${index === 0 ? 'text-[#ffd700]' : 'text-[#10b981]'}`}>{miembro.puntos_grupo}</div>
