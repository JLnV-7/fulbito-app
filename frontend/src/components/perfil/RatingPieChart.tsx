...
export function RatingPieChart({ userId }: RatingPieChartProps) {
    ...
    return (
        <div className="bg-[var(--card-bg)]/80 backdrop-blur-xl border border-[var(--card-border)]/50 rounded-[2rem] p-5 shadow-sm mb-4">
            <h3 className="text-[10px] font-black text-[#f59e0b] uppercase tracking-widest mb-2 flex items-center justify-center gap-2">
                <span className="text-sm">⭐</span> Distribución de Ratings
            </h3>
                            ...
                            <div key={item.rating} className="space-y-1">
                                <div className="flex justify-between text-xs font-semibold">
                                    <span className="text-[var(--text-muted)] flex gap-1 items-center">
                                        <span className="text-[#f59e0b]">★</span> {item.rating} {RATING_LABELS[item.rating]}
                                    </span>
