...
export function PartidosAmigosTab({ grupo }: PartidosAmigosTabProps) {
    const { partidos, loading, abrirVotacion, eliminarPartido, refetch } = usePartidosAmigos(grupo.id)
    const { user } = useAuth()
    const { showToast } = useToast()

    const [creando, setCreando] = useState(false)
    const [detalleVisible, setDetalleVisible] = useState<PartidoAmigo | null>(null)
    const [tabInicial, setTabInicial] = useState<'info' | 'votos' | 'resultados' | 'stats'>('info')
    ...
    const abrirDetalle = (p: PartidoAmigo, tab: 'info' | 'votos' | 'resultados' | 'stats') => {
        setDetalleVisible(p)
        setTabInicial(tab)
    }
    ...
    return (
        <>
                        ...
                        {borradores.map(p => {
                            ...
                            return (
                                <PartidoCard key={p.id} partido={p} variant="borrador">
                                    ...
                                    <div className="flex gap-2">
                                        <button
                                            onClick={() => abrirDetalle(p, 'info')}
                                            className="flex-1 bg-[var(--hover-bg)] text-[var(--foreground)] py-2.5 font-black capitalize tracking-widest text-xs border border-[var(--card-border)]"
                                            style={{ borderRadius: 'var(--radius)' }}
                                        >
                                            ✏️ Editar
                                        </button>
                                        ...
                                        <button
                                            ...
                                            className={`px-3 py-2.5 font-black text-xs capitalize tracking-widest border transition-all ${
                            ...
                            <PartidoCard key={p.id} partido={p} variant="finalizado">
                                {p.resultado_azul !== null && p.resultado_rojo !== null && (
                                    <p className="text-sm font-black capitalize tracking-tighter mb-2 italic">
                                        🔵 {p.resultado_azul} — {p.resultado_rojo} 🔴
                                    </p>
                                )}
                                <div className="flex gap-2">
                                    <button
                                        onClick={() => abrirDetalle(p, 'resultados')}
                                        className="flex-1 bg-[#2563eb] text-white py-2.5 font-black capitalize tracking-widest text-xs"
                                        style={{ borderRadius: 'var(--radius)' }}
                                    >
                                        📊 Ver Resultados
                                    </button>
                                    <button
                                        onClick={() => abrirDetalle(p, 'votos')}
                                        className="px-3 py-2.5 text-[10px] font-black capitalize tracking-widest border border-[var(--card-border)] text-[var(--text-muted)] hover:bg-[var(--hover-bg)]"
                                        style={{ borderRadius: 'var(--radius)' }}
                                    >
                                        🗳️
                                    </button>
            ...
            {/* Modal de Detalle — nuevo PartidoModal */}
            <AnimatePresence>
                {detalleVisible && (
                    <PartidoModal
...
function PartidoCard({ partido, variant, children }: {
    ...
    const formatFecha = (fecha: string, hora: string) => {
        ...
        return d.toLocaleDateString('es-AR', { weekday: 'short', day: 'numeric', month: 'short' }) + ` - ${hora.slice(0, 5)}`
    }
